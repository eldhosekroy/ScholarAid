...
from .forms import LoginForm, ScholarshipForm
...
def addscholar(request):
    # Ensure role is sponsor
    role = getattr(request.user, 'role', None) or ( "admin" if request.user.is_superuser else "student")
    if role != 'sponsor' and not request.user.is_superuser:
        messages.error(request, f'Only sponsors can create scholarships. Your current role is: {role}')
        return redirect('home')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                scholarship.sponsor = request.user
                scholarship.save()
                messages.success(request, f'Scholarship "{scholarship.name}" created successfully!')
                return redirect('ourscholar')
            except Exception as e:
                messages.error(request, f'Error saving scholarship: {str(e)}')
        else:
            # Report first error to user
            for field, errors in form.errors.items():
                messages.error(request, f"{field.capitalize()}: {errors[0]}")
    else:
        form = ScholarshipForm()

    return render(request, 'addscholar.html', {'form': form})
...
def delete_student(request, pk):
    student = get_object_or_404(StudentProfile, id=pk, role="student")
    student.delete()
...
@login_required
def examadmin(request):
    # Only sponsors and superusers can manage exams
    if not request.user.is_superuser and request.user.role != "sponsor":
        messages.error(request, "Only sponsors and admins can access exam management.")
        return redirect('home')
    if request.method == "POST":
        ...
        messages.success(request, f'Exam "{title}" and {i} questions created successfully!')
    ...
    if request.user.is_superuser:
        ...
        # Only show exams for scholarships owned by this sponsor
        exams = Exam.objects.filter(scholarship__sponsor=request.user)
        scholarships = Scholarship.objects.filter(sponsor=request.user)
...
def edit_exam(request, exam_id):
    ...
    # Check permission: superuser or the sponsor who owns the scholarship
    if not request.user.is_superuser and (request.user.role != "sponsor" or (exam.scholarship and exam.scholarship.sponsor != request.user)):
    ...
    if request.method == "POST":
        ...
        exam.passing_score = request.POST.get("passing_score")
        
        scholarship_id = request.POST.get("scholarship_id")
        if scholarship_id:
            exam.scholarship = get_object_or_404(Scholarship, id=scholarship_id)
        
        exam.save()

        # Update questions
        # First, delete existing questions and options (simpler regrouping)
        # OR better: update existing ones. But deleting and re-creating is easier for dynamic forms.
...
def exam(request, exam_id):
    exam = get_object_or_404(Exam, id=exam_id)
    # Check if user is an admin or the sponsor of this exam - they should be allowed to preview
    is_admin_or_sponsor = request.user.is_superuser or (request.user.role == "sponsor" and exam.scholarship and exam.scholarship.sponsor == request.user)
    
    # Check if student already taken this exam (only for students, not for admin preview)
    if not is_admin_or_sponsor:
        application = ScholarshipApplication.objects.filter(student=request.user, scholarship=exam.scholarship).first()
        if application and application.has_taken_exam and request.method == "GET":
            messages.info(request, "You have already completed this exam.")
            return redirect('registered_scholarships')
    ...
    if request.method == "POST":
        ...
        # ✅ Update ScholarshipApplication ONLY if it's a student taking the exam
        if not is_admin_or_sponsor:
            try:
...
def attend_exam(request, scholarship_id):
    ...
    # Correctly find the exam associated with this scholarship
    exam = Exam.objects.filter(scholarship=scholarship).first()
    
    if exam:
        return redirect('exam', exam_id=exam.id)
    else:
        messages.error(request, 'The exam for this scholarship has not been set up by the sponsor yet.')
        return redirect('registered_scholarships')
